...
#else
    { "none",		0,		gettext_noop("None/Disabled") },
#endif
...
void Video_MakeSettings(std::vector <MDFNSetting> &settings)
{
 ...
 for(unsigned int i = 0; i < MDFNSystems.size() + 1; i++)
 {
  ...
  BuildSystemSetting(&setting, sysname, "shader.goat.pat", gettext_noop("Mask pattern."), nullptr, MDFNST_ENUM, "none", NULL, NULL, NULL, NULL, GoatPat_List);
  settings.push_back(setting);
